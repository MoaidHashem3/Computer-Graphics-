...
#pragma omp parallel for
	...
	inline void aabb<VB>::add_triangle(const triangle<VB> triangle)
	{
		...
		aabb_min = max(aabb_max, triangle.a);
		aabb_min = max(aabb_max, triangle.b);
		aabb_min = max(aabb_max, triangle.c);
	...
	{
		float3 inv_ray_diraction = float3(1.f) / ray.direction;
		float3 t0 = (aabb_max - ray.position) * inv_ray_diraction;
		float3 t1 = (aabb_min - ray.position) * inv_ray_diraction;
